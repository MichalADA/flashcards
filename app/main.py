...
class ListeningScreenWrapper(Screen):
    def __init__(self, manager, db_instance, **kwargs):
        super(ListeningScreenWrapper, self).__init__(**kwargs)
        self.name = 'listening'
        self.add_widget(ListeningScreen(manager=manager, db_instance=db_instance))
...
class MainApp(App):
    def build(self):
        ...
        sm.add_widget(LearnedFlashcardsScreenWrapper(manager=sm, name='learned_flashcards'))
        sm.add_widget(GrammarScreenWrapper(manager=sm, name='grammar'))
        sm.add_widget(ListeningScreenWrapper(
            manager=sm,
            db_instance=self.db,
            name='listening'
        ))
